...
#  - trigger snapshot (with 500-retry)
#  - poll until ready (bounded)
#  - download CSV/JSON
#  - normalize to a compact schema (+PULL_DATE)
...
# API key: prefer env var, otherwise keep a literal until you move to secrets.
...
def trigger_snapshot() -> str:
    """
    Trigger a new snapshot with simple retry on 5xx.
...
def get_snapshot_status(snapshot_id: str) -> str:
    ...
    Some Bright Data payloads omit 'status' but allow CSV HEAD; treat that as 'ready'.
    """
    url = f"{BASE}/snapshot/{snapshot_id}"
    try:
        r = SESSION.get(url, headers=HEADERS_JSON, timeout=60)
        # Sometimes BD returns 200 with partial JSON or even a record;
        # try to parse, but fall back to text sniffing.
        ...
        # Heuristic: if CSV is already available, consider ready.
        if _csv_available(snapshot_id):
            return "ready"

        # If payload looks like a data record, snapshot is still building → running.
...
def poll_until_ready(snapshot_id: str, max_wait_minutes: int, poll_every: int) -> None:
    """
    Poll snapshot until ready or timeout; raise on failure/timeout.
    """
...
def normalize_csv(raw_csv_path: Path, out_dir: Path) -> Path:
    ...
    with open(raw_csv_path, "r", encoding="utf-8", newline="") as f_in, \
        ...
        # We always write the compact normalized schema + PULL_DATE
...
def alaska_filter(csv_path: Path, out_path: Optional[Path] = None) -> Optional[Path]:
    ...
    Requires 'zipcode' column; if absent, it no-ops.
    """
    try:
        import pandas as pd
    except Exception:
...
def run_pipeline(existing_snapshot_id: Optional[str],
    ...
    if not API_KEY or API_KEY == "PASTE_YOUR_KEY_HERE":
        die("Set BRIGHTDATA_API_KEY env var (or replace placeholder API key).")
