...
#   BRIGHTDATA_INPUT_CSV         (prebuilt CSV; skipped if --search-file is used)
...
def normalize_csv(raw_csv_path: Path, out_dir: Path) -> Path:
    ...
    Normalize the Bright Data CSV into:
      - One combined file for all records: WM_ALL_MM_YY.csv
      - One file per store: WM_<store_id>_MM_YY.csv
    ...
    print(f"[{_ts()}] [OK] Saved combined normalized CSV → {all_path}")
    print(f"[{_ts()}] [OK] Saved {len(store_writers)} store-level CSV files in {out_dir}")
...
def run_pipeline(
    ...
    if existing_snapshot_id:
        ...
        if search_file is not None and search_file.exists():
            ...
            die("No search-file provided and no --items/--zips specified. "
                "BRIGHTDATA_INPUT_CSV fallback is disabled to prevent uploading .txt files.")
