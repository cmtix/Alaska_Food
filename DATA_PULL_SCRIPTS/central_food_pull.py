...
- Ensures:
    ...
    - BRIGHTDATA_API_KEY is available for WM
...
def find_latest_py(folder_abs: str) -> str | None:
    """
    Recursively find the newest .py file under folder_abs by modification time.
    Returns the path as a string, or None if none found.
    """
...
# Locations of store-specific Python scripts
acc_script = find_latest_py(os.path.join(PROJ, "DATA_PULL_SCRIPTS", "ACC"))
cs_script = find_latest_py(os.path.join(PROJ, "DATA_PULL_SCRIPTS", "CS"))
fm_script = find_latest_py(os.path.join(PROJ, "DATA_PULL_SCRIPTS", "FM"))
wm_script = find_latest_py(
    os.path.join(PROJ, "DATA_PULL_SCRIPTS", "WM", "Bright_Data", "Scripts")
)
...
def main() -> int:
    # Control switches (mirror R defaults; override via env or BAT)
    RUN_ACC = parse_bool(os.getenv("RUN_ACC", "TRUE"), default=True)
    RUN_CS = parse_bool(os.getenv("RUN_CS", "FALSE"), default=False)
    RUN_FM = parse_bool(os.getenv("RUN_FM", "TRUE"), default=True)
    RUN_WM = parse_bool(os.getenv("RUN_WM", "TRUE"), default=True)
    ...
    wm_env = {}
    if RUN_WM:
        bright_key = os.getenv("BRIGHTDATA_API_KEY", "").strip()
        if not bright_key:
            print(
                "ERROR: BRIGHTDATA_API_KEY is not set, but RUN_WM=TRUE.\n"
                "Set BRIGHTDATA_API_KEY in run_food_pull.bat or OS env before running.",
                file=sys.stderr,
            )
            return 1
        ...
        out_dir = os.getenv(
            "BRIGHTDATA_OUT_DIR",
            os.path.join(PROJ, "DATA_PULL_SCRIPTS", "WM", "Bright_Data", "Outputs"),
        )
        dataset_id = os.getenv("BRIGHTDATA_DATASET_ID", "gd_m693oc1r1gebnayxq")
  

        wm_env = {
            "STORE": "WM",
            "RUN_DATE": RUN_DATE,
            "BRIGHTDATA_API_KEY": bright_key,
            "BRIGHTDATA_USE_LAST": use_last,
            "BRIGHTDATA_MAX_WAIT_MIN": max_wait,
            "BRIGHTDATA_POLL_EVERY_S": poll_every,
            "BRIGHTDATA_OUT_DIR": out_dir,
            "BRIGHTDATA_DATASET_ID": dataset_id,
        
        }

        # Only pass BRIGHTDATA_SNAPSHOT_ID if explicitly set in the environment / BAT
        if snapshot_id:
            wm_env["BRIGHTDATA_SNAPSHOT_ID"] = snapshot_id


        print(
            "WM env: BRIGHTDATA_API_KEY=",
            mask(bright_key),
            file=sys.stdout,
            flush=True,
        )
